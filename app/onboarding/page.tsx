...
export default function OnboardingPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/partnerships/import/upload`, formData, {
  ...
  const handleImport = async () => {
    ...
    setUploading(true);
    try {
      const response = await axios.post(`${API_URL}/api/partnerships/import/execute`, {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please select an organization from the dashboard to continue.</p>
          <button
            onClick={() => window.location.href = '/dashboard'}
            className="mt-4 px-6 py-2 bg-gradient-to-r from-[#60a5fa] to-[#60e1fa] text-white rounded-lg"
          >
            Go to Dashboard
          </button>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-gray-400 mt-1">Importing to: {currentOrg.name}</p>
            </div>
          </div>
        </div>
      </header>
            ...
            <div className="p-6">
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-gradient-to-r from-[#60a5fa] to-[#60e1fa] text-white rounded-lg hover:opacity-90 disabled:opacity-50"
                >
                  {uploading ? 'Importing...' : `Import ${importData.length} Partnerships to ${currentOrg.name}`}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">Partnerships imported to {currentOrg.name}</p>
                </div>
              </div>
            </div>
